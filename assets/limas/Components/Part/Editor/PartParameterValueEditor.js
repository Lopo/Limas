Ext.define('Limas.PartParameterValueEditor', {
	...
	initComponent: function () {
		...
		this.unitFilter = Ext.create('Limas.util.Filter', {
			property: '@id',
			operator: 'in',
			value: []
		});
	...
	onPartParameterSelect: function (combo, record) {
		...
		if (record.get('unitName') !== null) {
			let unit = this.down('#unit').getStore().findRecord('name', record.get('unitName'), 0, false, true, true);
		...
		let valueType = record.get('valueType');
	...
	onUnitChange: function (combo, newValue) {
		...
		if (newValue instanceof Limas.Entity.Unit) {
			prefixes = newValue.prefixes().getData();
			for (let j = 0; j < prefixes.getCount(); j++) {
