Ext.define('Limas.PartFilterPanel', {
	...
	_onFilterRemove: function () {
		...
		for (let i = 0; i < this.filterControls.length; i++) {
			...
			if (filterPlugin instanceof Limas.Util.FilterPlugin) {
	...
	/**
	 * Applies the parameters from the filter panel to the proxy, then reload the store to refresh the grid
	 */
	onApply: function () {
		let i;
	...
	/**
	 * Resets the fields to their original values, then call onApply() to reload the store
	 */
	onReset: function () {
	...
	createFilterFields: function () {
		...
		this.storageLocationFilter = Ext.create('Limas.StorageLocationComboBox', {
			flex: 1,
			forceSelection: true,
			plugins: [
						...
						getFilterFn: function () {
							if (this.partManager !== null) {
								if (this.categoryFilter.getValue().category === 'all' && this.partManager.getSelectedCategory() !== null && !this.partManager.getSelectedCategory().isRoot()) {
									...
									let selectedCategory = this.partManager.getSelectedCategory();
		...
		this.manufacturerFilterCombo = Ext.create('Limas.ManufacturerComboBox', {
			flex: 1,
			plugins: [
				Ext.create('Limas.Util.FilterPlugin', {
					getFilterFn: function () {
		...
		this.distributorFilterCombo = Ext.create('Limas.DistributorComboBox', {
			flex: 1,
			plugins: [
				Ext.create('Limas.Util.FilterPlugin', {
					getFilterFn: function () {
		...
		this.footprintFilterCombo = Ext.create('Limas.FootprintComboBox', {
			flex: 1,
			plugins: [
				Ext.create('Limas.Util.FilterPlugin', {
					getFilterFn: function () {
		...
		this.internalIdFilter = Ext.create('Ext.form.field.Text', {
			...
			anchor: '100%',
			itemId: 'idField',
			qtip: i18n('The first number is the ID in decimal, the second number is the ID in base36. To search in base36 format you need to prefix the search string with #, example: #15y'),
			plugins: [
				Ext.create('Limas.Util.FilterPlugin', {
					getFilterFn: function () {
						let idstr = this.internalIdFilter.getValue(),
							idint;
