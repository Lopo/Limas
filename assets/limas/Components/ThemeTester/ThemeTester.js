Ext.define('Limas.ThemeTester', {
		...
		return this.applySizing('panelWithToolbars', {
			...
			bbar: {
				...
				items: [
					...
					' ',
					'-',
					this.isTouchTheme ? null : {text: 'Button'},
					{
	...
	getProgressBar: function () {
		let progressbar = Ext.widget('progressbar', {
			value: 0.5
		});
	...
	doThemes: function () {
		let time = Ext.perf.getTimestamp(),
			maskedPanel,
			items = [
				this.getBasicPanel(),
		...
		this.items = Ext.create('Ext.container.Container',
			this.applySizing('mainContainer', {
				id: 'main-container',
				layout: 'absolute',
				items: items
			})
