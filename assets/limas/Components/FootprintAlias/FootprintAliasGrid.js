/**
 * Admin grid for the InfoProvider aggregator's `FootprintAlias` table.
 * Lets the user review auto-discovered package strings, assign a Footprint
 * to unverified rows inline, and flip the verified flag.
 *
...
Ext.define('Limas.FootprintAliasGrid', {
	...
	initComponent: function () {
		this.columns = [
			...
			{
				header: i18n('Footprint'), dataIndex: 'footprint', flex: 2,
				filter: {type: 'string'},
				renderer: function (v, m, rec) {
					// returnObject combo writes an ExtJS Model into the cell;
					// v.name would be undefined. Prefer the model getter.
					if (rec && typeof rec.getFootprint === 'function') {
						let f = rec.getFootprint();
						if (f && typeof f.get === 'function') return Ext.htmlEncode(f.get('name') || '');
					}
					if (!v) {
