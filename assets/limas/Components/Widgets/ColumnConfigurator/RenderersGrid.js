Ext.define('Limas.Components.Widgets.ColumnConfigurator.RenderersGrid', {
	extend: 'Ext.grid.Panel',
	xtype: 'limas.renderersGrid',
	...
	initComponent: function () {
		...
		this.columns = [{
			...
			renderer: function (val, q, rec) {
				let targetClass = Limas.Components.Grid.Renderers.RendererRegistry.lookupRenderer(rec.get('rtype'));
	...
	configure: function (record) {
		let i = Ext.create('Limas.Components.Widgets.ColumnConfigurator.RendererConfigurationForm', {
	...
	setSourceModel: function (sourceModel) {
		...
		let rendererClasses = Limas.Components.Grid.Renderers.RendererRegistry.getRenderersForEntity(this.sourceModel.$className);
		...
		for (let i = 0; i < rendererClasses.length; i++) {
			this.rendererStore.add({
				...
				rendererType: Limas.Components.Grid.Renderers.RendererRegistry.getRType(rendererClasses[i])
			});
		}
	},
