Ext.define('Limas.Components.Widgets.ColumnConfigurator.ColumnListGrid', {
	...
	store: {
		model: 'Limas.Models.ColumnConfiguration',
		sorters: [{
			property: 'index',
			direction: 'ASC'
		}]
	...
	addField: function () {
		...
		let j = Ext.create('Limas.Models.ColumnConfiguration', {
			index: max + 1,
			widthMode: 'flex',
			flex: 1
		});
	...
	deleteField: function () {
		let oldIdx = this.getStore().indexOf(this.getSelection()[0]);
	...
	moveDown: function () {
		let record, nextRecord, sort, myIndex;
	...
	moveUp: function () {
		let record, previousRecord, sort, myIndex;
