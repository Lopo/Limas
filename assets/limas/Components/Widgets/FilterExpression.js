Ext.define('Limas.Widgets.FilterExpression', {
	extend: 'Ext.form.Panel',
	xtype: 'limas.filterexpression',
	...
	onApplyClick: function () {
		...
		if (this.down('#operator').getValue().get('operator') === 'in') {
			values = [];
			for (let j = 0; j < this.down('#values').getStore().getCount(); j++) {
		...
		this.fireEvent('applyfilter', Ext.create('Limas.util.Filter', {
	...
	validateApplyButton: function () {
		let applyButton = this.down('#apply');
	...
	onFieldSelectClick: function () {
		this.modelFieldSelectorWindow = Ext.create('Limas.Components.Widgets.FieldSelectorWindow', {
