Ext.define('Limas.BatchJobEditor', {
	extend: 'Limas.Editor',
	...
	initComponent: function () {
		...
		this.down('#batchJobUpdateFields').getSelectionModel().on('selectionchange', this.onUpdateSelectionChange, this);
	},
	beforeFieldSelection: function (selectorField) {
		let sourceModel = Ext.ClassManager.get(this.down('#baseEntity').getValue());
		if (sourceModel === null) {
			Ext.Msg.alert(i18n('Base Entity not selected'), i18n('You need to select a base entity to perform the query against'));
			return;
		}
	...
	onAddQueryFieldButtonClick: function () {
		...
		this.addFilterWindow = Ext.create('Limas.Widgets.FilterExpressionWindow', {
	...
	onAddUpdateFieldButtonClick: function () {
		...
		Ext.create('Limas.Components.BatchJob.BatchJobUpdateExpressionWindow', {
			sourceModel: sourceModel,
			listeners: {
	...
	onDeleteQueryFieldButtonClick: function () {
		let selection = this.down('#batchJobQueryFields').getSelectionModel().getSelection()[0];
		if (selection) {
			this.down('#batchJobQueryFields').getStore().remove(selection);
		}
	},
	onDeleteUpdateFieldButtonClick: function () {
		let selection = this.down('#batchJobUpdateFields').getSelectionModel().getSelection()[0];
		if (selection) {
	...
	onEditStart: function () {
		this.down('#batchJobQueryFields').bindStore(this.record.batchJobQueryFields());
		this.down('#batchJobUpdateFields').bindStore(this.record.batchJobUpdateFields());
	}
});
