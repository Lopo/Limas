 ...
 */
Ext.define('Limas.Auth.LoginManager', {
	...
	constructor: function (config) {
		...
		this.provider = Limas.Auth.AuthenticationProvider.getAuthenticationProvider();
		this.provider.on('authenticate', this.onAuthenticate, this);

		if (!this.config.autoLogin) {
			this.loginDialog = Ext.create('Limas.LoginDialog');
	 ...
	 */
	logout: function () {
		Limas.Entity.User.callGetCollectionAction('logout',
			{},
	...
	/**
	 * Callback for the logout action. Fires the logout event, which destroys all windows.
	 */
	onLogout: function () {
		this.loggedIn = false;
