Ext.define('Limas.Auth.AuthenticationProvider', {
	...
	authenticate: function () {
		Limas.Entity.User.callPostCollectionAction('login',
			{},
	...
	onLogin: function (options, success, response) {
		if (response.status === 401) {
			...
			let reader = Limas.Entity.User.getProxy().getReader();
			this.setUser(reader.read(response, {
	...
	statics: {
		...
		getAuthenticationProvider: function () {
			if (!(this.authenticationProvider instanceof Limas.Auth.AuthenticationProvider)) {
				this.authenticationProvider = Ext.create('Limas.Auth.AuthenticationProvider');
			}
