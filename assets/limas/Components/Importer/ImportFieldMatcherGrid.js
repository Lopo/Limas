Ext.define('Limas.Importer.ImportFieldMatcherGrid', {
	...
	setModel: function (model, ignoreModel) {
		...
		this.ignoreModel = ignoreModel === undefined ? null : ignoreModel;
	},
	...
	getImporterConfig: function () {
		let data = this.getStore().getData(),
			serializedRecords = [],
			serializedRecord;
	...
	onAddMatchField: function () {
		let excludeFields = [];
		for (let j = 0; j < this.store.getCount(); j++) {
		...
		let modelFieldSelector = Ext.create({
			...
			listeners: {
				selectionchange: function (selectionModel, selected) {
					let addFieldButton = this.up('#matchFieldWindow').down('#addSelectedField');
