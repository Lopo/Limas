Ext.define('Limas.Importer.ImporterEntityConfiguration', {
	...
	onImportBehaviourChange: function () {
		if (this.getForm().getFieldValues().importBehaviour === 'matchData') {
	...
	getImporterConfig: function () {
		let config = this.getForm().getFieldValues();
	...
	setImporterConfig: function (config) {
		...
		this.down('#importFieldMatcherGrid').setImporterConfig(config.hasOwnProperty('matchers') ? this.importerConfig.matchers : {});
