Ext.define('Limas.Importer.Importer', {
	...
	bbar: [
		...
		{
			xtype: 'presetcombo',
			model: 'Limas.Entity.ImportPreset',
	...
	onFileUploaded: function (data) {
		this.loadData(Ext.create('Limas.Entity.TempUploadedFile', data).getId());
	},
	...
	onFieldChange: function (selectionModel, selected) {
		if (selected.length === 1) {
			...
			if (Ext.isFunction(this.down('#configurationCards').getLayout().getActiveItem().reconfigureColumns)) {
				this.down('#configurationCards').getLayout().getActiveItem().reconfigureColumns(this.importColumnsStore);
			}
		}
	},
	...
	reconfigureGrid: function (data) {
		...
		let store = Ext.create('Ext.data.Store', fieldConfig),
			recordData = [];
	...
	validateConfigNode: function (node) {
		let configuration = node.data.data.configuration,
			recurse = false;
			...
			case 'field':
				if (configuration.fieldConfiguration && configuration.fieldConfiguration === 'copyFrom') {
					if (this.down('#sourceFileGrid').getColumns().length - 1 < configuration.copyFromField) {
						this.appendError(node, i18n('The selected CSV file does not contain enough columns to fulfill the configuration'));
					}
				}
						...
						switch (configuration.fieldConfiguration) {
							...
							case 'copyFrom':
								if (configuration.copyFromField === '') {
									this.appendError(node, i18n('The field is configured to copy a value from the source file, but no source file field was configured'));
								}
								break;
							default:
								break;
	...
	appendError: function (node, error) {
		...
		let title = i18n('Errors') + ' (' + this.down('#errorsGrid').getStore().getCount() + ')';
