Ext.define('Limas.StatisticsChart', {
	...
	series: [
		{
			...
			tooltip: {
				...
				renderer: function (tip, item) {
					tip.update(
						Ext.Date.format(item.get('start'), 'Y-m-d') + ': ' + item.get('parts') + ' ' + i18n('Parts'));
				}
			},
			title: i18n('Parts'),
			markerConfig: {
			...
			tooltip: {
				trackMouse: true,
				renderer: function (tip, item) {
					tip.update(Ext.Date.format(item.get('start'), 'Y-m-d') + ': ' + item.get('categories') + ' ' + i18n('Categories'));
				}
			},
			axis: 'left',
	...
	store: {
		...
		proxy: {
			...
			url: Limas.getBasePath() + '/api/statistics/sampled',
			extraParams: {
