Ext.define('Limas.StatisticsChartPanel', {
	...
	reloadDates: function () {
		Ext.Ajax.request({
			url: Limas.getBasePath() + '/api/statistics/range',
			method: 'GET',
	...
	onReloadDates: function (options, success, response) {
		...
		let start = Ext.Date.parse(data.startDate, 'Y-m-d H:i:s'),
			end = Ext.Date.parse(data.endDate, 'Y-m-d H:i:s');
