...
Ext.define('Limas.EditorGrid', {
	...
	initComponent: function () {
		...
		let targetField = this.titleProperty;

		this.searchField = Ext.create('Limas.form.field.SearchField', {
			store: this.store,
		...
		this.appliedFiltersToolbar = Ext.create('Limas.Grid.AppliedFiltersToolbar', {
			dock: 'bottom',
	...
	_onFilterChange: function () {
		let filters = this.getStore().getFilters();
		if (filters.length > 0) {
	...
	reassignPageSize: function () {
		...
		let numRecords = Math.floor(this.getView().getHeight() / this.automaticPageSizeRowHeight);

		if (numRecords < 1) {
			numRecords = 1;
		}

		let oldStartIndex = this.store.pageSize * this.store.currentPage;

		this.store.pageSize = numRecords;

		let newStartPage = Math.floor(oldStartIndex / numRecords);
