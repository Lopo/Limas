Ext.define('Limas.ExceptionWindow', {
	...
	_showException: function (exception, response) {
		...
		let fullDetails = exception.getTitle();
		...
		if (exception.getTrace()) {
			let traceData = this.convertTraceToTree(exception.getTrace()),
		...
		for (let j in window.parameters) {
	...
	statics: {
		...
		showException: function (response) {
			if (!Limas.ExceptionWindow.activeInstance) {
				Limas.ExceptionWindow.activeInstance = new Limas.ExceptionWindow();
			}
