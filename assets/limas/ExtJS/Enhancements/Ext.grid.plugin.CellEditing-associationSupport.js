 ...
 */
Ext.define('Limas.grid.plugin.CellEditing', {
	...
	onEditComplete: function (ed, value, startValue) {
		...
		// Only update the record if the new value is different than the startValue. When the view refreshes its el will gain focus
		if (!record.isEqual(value, startValue)) {
			if (record.hasField(context.column.dataIndex)) {
				...
				if (record.associations[context.column.dataIndex]) {
					let setterName = record.associations[context.column.dataIndex].setterName;
